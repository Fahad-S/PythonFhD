...
# for files in os.walk(rootdir):
    ...
    for file in files:
        ...
        if(file[0]=='.'):
            files.pop(0)
            continue
        if(skip_second_reading=='true'):
            continue
        else:
            skip_second_reading='true'
...
# for c in range(0, len(file_names)):
        ...
        chr_num = chromosome.replace('chr','')#chr_file_name[chr_file_name.find("r")+1:chr_file_name.find("_")]
        ...
        chro_outfile = open(directory + name1+'_Analyzed.txt', 'w')
        ...
        chro_indataUnsorted2=[]
        for line in chro_indataUnsorted1:
             line = line.split('\t')
             chro_indataUnsorted2.append(line)
        chro_indataUnsorted2 = sorted(chro_indataUnsorted2, key = lambda x : ( x[1], x[2], int(x[7])))

        chro_infile = open(subdir+'/Sorted_'+files[0], 'w')
        chro_infile.write(header)
        for line in chro_indataUnsorted2:
            newLine= '\t'.join(line)
            # newLine = newLine + '\n'
            chro_infile.write(str(newLine))

        ''' End of external sort '''


        chro_infile = open(subdir+'/Sorted_'+files[0], 'r')
    ...
    #                        exit()
        ...
        full_data = open(directory + name1+'_Final.txt', 'w')
        ...
        exons_data = open(directory + name1+'_Final_Exons.txt', 'w')
        ...
        introns_data = open(directory + name1+'_Final_Introns.txt', 'w')
        ...
        chro_infile = open(directory+name1+'_Analyzed.txt', 'r')
